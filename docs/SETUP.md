...
4. **Labels** are bootstrapped by `/orch-setup`. For the copy-in path run `scripts/orchestration/bootstrap-labels.sh .github/labels.yml`.
   - **Re-run it after every ganpan upgrade.** Bootstrapping happens *once*, at setup, so a repo installed before a label was added to `labels.yml` never receives that label — and the gap only surfaces when a lane's `--add-label` write fails mid-run (this is how `status:needs-decision`, added with the Reviewer decision gate, went missing here; issue #81). The bootstrap is idempotent — `gh label create --force` is create-or-update and never deletes or renames — so re-running it on an up-to-date repo is a no-op.
   - **Check for drift** at any time with `scripts/orchestration/labels-check.sh` (read-only; exits 1 and names the missing labels). `/ganpan:update` runs it as part of its advisory.
...
6. **Branch protection on `main`:** require 1 human review (or CODEOWNERS), no force-push, no direct push, **include administrators**, restrict review dismissal. Bot token must **not** be admin.
   - **Constraint — private repo on GitHub Free:** the branch-protection API is a paid feature, so on a **private** repo under the **Free** plan `gh api repos/:repo/branches/main/protection` always returns `403 "Upgrade to GitHub Pro or make this repository public…"` even when no protection exists. That means you cannot actually configure protection here, **and** `reviewer.autoMerge` cannot work — it reads that 403 as an inconclusive probe and fails closed (`protect-check-failed`), leaving passing PRs stuck in `in-review`. To use auto-merge on such a repo, either make it public / upgrade to Pro/Team, or (accepting that a Free private repo cannot have protection) set `reviewer.autoMergePrivatePlanWorkaround: true` — an opt-in that treats **only** that exact 403 as "unprotected". Default `false`; all other inconclusive probes still fail closed and real protection is never bypassed.
   - **Setup-time detection:** `/orch-setup` runs `scripts/orchestration/visibility-check.sh` before printing the checklist, and you can run it any time (read-only; exit 1 = the advisory applies, not a failure). It reads `gh api repos/:repo --jq .private` and, on a private repo, prints the 403 trap and the three fixes above. It deliberately does **not** detect your plan — the owner-plan field is not reliably exposed to a fine-grained token — so on a paid private repo you can simply ignore the advisory and leave the flag at `false`.
