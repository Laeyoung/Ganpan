...
2. 통과 → `status:qa`→`status:done`, **이슈 close**(`gh issue close <n> --reason completed`), `project_sync <n> "Done"`(§5.1), worktree 정리. QA가 종료(close)를 책임진다 — PR의 `Closes #<n>` 자동 close에 의존하지 않는다(머지된 PR 본문에 키워드가 없으면 이슈가 열린 채 `status:done`만 남기 때문). 이미 닫힌 이슈를 close하는 것은 무해한 no-op. 닫힌(`status:done`) 이슈를 사람이 재오픈하는 것은 자동 레인 처리 밖이다 — 어떤 레인도 재오픈된 done 이슈를 재개하지 않으며, 사람이 `status:triage`를 다시 적용해야 파이프라인으로 재유입된다.
