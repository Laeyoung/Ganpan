...
- **새(new) 신뢰 입력:** 봇의 가장 최근 `rework-requested:` / `decision-requested:` / `decision-clarify:` / `decision-resolved:` 마커(이 중 가장 나중) **이후**에 신뢰 사람이 게시한 코멘트. `decision-resolved:`(종료 마커)도 윈도우를 리셋한다 — 게이트 해소 후 해소 이전 답변(예: 옛 "진행")이 다음 틱까지 남아 이후의 신뢰 "수정" 답변과 합쳐져 *허위 진행+수정→clarify* 로 오염되는 것을 막기 위함이다(해소 후에는 해소 이후 답변만 센다). 단, `decision-resolved:` 자체는 *답변*으로는 여전히 inert(무효)다 — 봇 마커이지 사람 답변이 아니므로 분류 대상이 아니다.
