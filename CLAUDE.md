...
- **Engine scripts keep stdout clean** — a script's stdout *is* its return value (lanes capture it via `ISSUE=$(claim.sh)`, `ANSWERS=$(trusted-answers.sh …)`, etc.). `gh issue edit/comment/create`/`pr create` print the resource URL to stdout even non-interactively, so **redirect every mutating `gh` write to `>/dev/null`** (errors still surface on stderr); send any diagnostics through `log` (stderr), never bare `echo`. This holds even for scripts not currently captured (e.g. `reclaim.sh`) so the contract can't rot if a caller later wraps them in `$(…)`. Regression-test it with the `GH_EMIT_WRITE_URL=1` gh-stub knob, asserting the leaked `STUB-URL` never reaches the script's stdout.
- `assets/CLAUDE.md` is shipped to users — editing it changes deploy output, not this repo's dev rules.
- **Keep engine-script stdout clean for the return value.** Any script whose stdout is captured via `$(…)` (e.g. `ISSUE=$(claim.sh)`, `AM=$(auto-merge.sh)`, `case "$(unblock-check.sh)"`) must emit **only** its return token on stdout. Mutating `gh` writes (`gh issue edit|comment|create`, `gh pr create|merge`, `gh label create`, `gh project item-edit`, `gh api --method POST|PUT|PATCH|DELETE`) print the resource URL/confirmation to stdout on success even non-interactively — send that to `/dev/null` (`>/dev/null`, keeping stderr open for `|| log WARN`) or capture it into a local (`out=$(gh … 2>&1)`). A leaked URL corrupts the captured value (the PR #28 / #29 bug class). Exception: `bootstrap-labels.sh` deliberately prints per-label progress to stdout — that is human-facing setup output, not a captured return value. Regression-guard new captured + mutating scripts with the `GH_EMIT_WRITE_URL` stub pattern (`tests/orchestration/helpers/gh-stub.sh`).
