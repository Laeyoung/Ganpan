...
## Routing actions
**Review comment format.** Every PR narrative (`gh pr comment` in R-A and R-D) must be **structured GitHub-flavoured markdown**, never one dense run-on paragraph: a lead line stating this is a bot review and not an approval (`리뷰 (봇, 승인 아님 — 머지는 사람이 진행):`), **bold section headings** (`**통과 기준 충족** ✅`, `**변경 요청**`, `**참고**`) each followed by a **bullet list** (one point per `-` bullet, not comma-joined prose), `✅`/`⚠️`/`❌` verdict markers, and backticks for files/labels. R-D closes with an explicit human-merge line; R-A leads with a `**변경 요청**` heading and one bullet per change request. Keep the issue markers (`rework-requested:` / `merge-requested:`) lean and unformatted — only the PR comment carries the formatted narrative.
