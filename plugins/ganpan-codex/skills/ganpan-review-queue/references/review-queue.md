...
3. If changes are required, post the rework reason **to the PR**, and a lean bot-authored `rework-requested:` issue comment that mentions the PR; then move `status:in-review` to `status:in-progress`. If a prior merge request is still open, retract it on **both** the PR and the issue. Keep the bot assignee and worktree.
