...
반려(reverse) 전이: status:in-review → status:in-progress (Reviewer가 수정 요구 시)
```
...
- **`status:qa`** — 사람이 PR을 승인·머지함, QA 검증 대기 (머지 행위는 사람만 수행 — 2.3·6.3)
- **`status:done`** — 완료
- **`status:blocked`** — 사람 개입 필요 (의존성, 모호성, 반복 실패 등)

GitHub Project의 status 필드를 이 label과 동기화하면, 웹 UI에서 칸반 보드로 시각화된다.

> **전제(부트스트랩):** 위 7종 `status:*` 라벨(triage·agent-ready·in-progress·in-review·qa·done·blocked)은 워크플로 시작 전에 repo에 미리 생성돼 있어야 한다(`gh label create` 스크립트 또는 `.github/labels.yml`). 그렇지 않으면 모든 `gh issue edit --add-label` 호출이 실패한다. 이슈 유입 경로(누가 `status:triage`를 만드는가)와 Project↔label v1 동기화 방식은 9장 open question으로 둔다.
...
| 3 | **Reviewer** | `/loop 5m /review-queue` | `in-review` → (기준 충족 시 사람 승인·머지 요청 → 머지되면 `qa`, 또는 코멘트 후 `in-progress`로 반려) |
| 4 | **QA** | `/goal` 기반 (3.4 참고) | `qa` → `done` (또는 회귀 이슈 생성) |

> **lane 수는 가변이다.** 가장 단순한 출발점은 Coder lane 1개이고, 검토 부하가 늘면 Reviewer를 분리하는 식으로 점진 확장한다.

> **머지 행위자:** lane 표의 `in-review → qa` 전이에서 PR을 실제로 머지하는 것은 **사람**이다(2.3·6.3 human-in-the-loop). Reviewer lane(에이전트)은 1차 검토·인라인 코멘트·반려까지만 담당하고, 기준 충족 시 사람의 승인·머지를 요청한다. 에이전트는 자신이 연 PR을 스스로 승인·머지하지 않는다(6.3 참고).
...
1. `gh issue list --label "status:agent-ready" --json number,...` 로 후보 목록을 받아 **상위 N개 중 무작위로 1개**를 고른다. (`--limit 1`로 항상 큐 맨 앞만 집으면 동시 워커가 같은 이슈로 몰려 경합이 오히려 커진다.)
2. 해당 이슈에 `status:in-progress` 추가 + `status:agent-ready` 제거, **그리고 본인 식별자를 함께 남긴다** — 워커 봇 계정으로 `gh issue edit <n> --add-assignee <bot>` 하거나 `gh issue comment <n>` 로 고유 claim 토큰(워커 ID + 타임스탬프)을 기록한다. (라벨만으로는 "내가 집었는지"와 "남이 집었는지"가 구별되지 않으므로 식별자가 필수다.)
3. 라벨 전파 지연(수백 ms~수 초)을 감안해 **잠시 대기(예: 2~3초) 후 재확인**: `gh issue view <n> --json labels,assignees,comments` 로 (a) `status:in-progress`가 반영됐고 (b) **assignee가 정확히 1명이며 그것이 본인인지** 확인한다. assignee는 복수 등록이 가능하므로(두 워커가 동시에 add-assignee하면 둘 다 등록됨), "본인 포함" 확인만으로는 불충분하다 — assignee가 2명 이상이면 **claim 코멘트 토큰의 어휘 최솟값(lexicographically first)을 승자로** 삼는 타이-브레이크를 적용하고, 패자는 본인 assignee를 해제한 뒤 1번으로 돌아간다. 단, GitHub 코멘트 전파가 2~3초 안에 보장되지 않으므로(상대 워커의 claim 코멘트가 아직 안 보이면 양쪽 모두 자신을 승자로 오판할 수 있다), 타이-브레이크 판정 전 코멘트 읽기를 짧은 백오프로 수 회 재시도해 상대의 claim 코멘트가 전파됐는지(코멘트 수 증가) 확인한 뒤 판정한다. claim 토큰은 정렬 순서가 더 이른 시각을 먼저 오게 설계한다(예: `ISO8601-UTC-타임스탬프 + 워커ID`).
4. 식별자가 본인 것이면 작업 시작. 식별자가 다른 워커 것이거나 모호하면(라벨이 안 보이거나 충돌) **claim 실패로 간주하고** 1번으로 돌아가 다른 이슈를 선택한다.

이는 Hermes Kanban 디스패처의 "원자적 claim"을 GitHub label + assignee로 흉내 낸 것이다. 완벽한 원자성은 아니지만(2~3번 사이에 잔존하는 좁은 경합 창이 있을 수 있음), 식별자 확인 + 충돌 시 재시도로 단일 개발 머신에서 소수 lane을 돌리는 규모에서는 충분하다. claim 충돌 가능성을 더 낮추려면 lane별로 담당 이슈 범위를 나누거나(예: 라벨·assignee 기준), 동시 Coder lane을 1개로 제한한다.
...
- **`/review-queue`** — `status:in-review` PR의 diff를 읽고 인라인 코멘트, 기준 충족 시 **사람에게 승인·머지를 요청**(에이전트가 직접 머지·자기승인하지 않음 — 6.3), 사람이 머지하면(매 폴링 시 `gh pr view <n> --json state,mergedAt` 로 merged 여부 확인) `status:qa`로 전이, 수정 필요 시 코멘트 후 `status:in-progress`로 반려.
...
### 6.1 동시성 경합
label 기반 claim은 완벽한 원자성이 아니다. → 동시 Coder lane을 제한하거나, lane별 이슈 범위를 분할. claim 시 본인 식별자(assignee/claim 토큰) 기록 + 대기 후 재확인 단계 필수(3.3).

### 6.2 검토 규율 붕괴 ("vibe coding으로의 미끄러짐)
에이전트를 많이 띄울수록 "그냥 머지하고 넘어가자"는 유혹이 커진다(GeekNews 커뮤니티가 지적한 실제 현상). → **검토를 진짜 제약(WIP 한도)으로 관리**: `status:in-review` + `status:qa` 합계에 상한(예: 5~10개)을 두고, 초과 시 Coder lane을 멈춘다. **구현**: `/work-issue` 커맨드의 첫 단계에서 `status:in-review`와 `status:qa` 라벨 이슈 수를 각각 세어(`gh issue list --label ... --limit 1000 | wc -l` — `gh issue list` 기본 limit 30에 걸려 누락되지 않도록 충분히 큰 값을 지정) 합산하고, 상한을 넘으면 claim 없이 그 턴을 no-op으로 종료한다(다음 `/loop` 주기에 재확인). 전통 칸반의 WIP 제한 원칙을 "사람 검토" 컬럼에 적용하는 것.
...
#### (3) 토큰 격리·branch protection이 막지 못하는 잔여 위험

위 두 층으로도 막히지 않는 위험과 대응:

- **비보호 브랜치 조작**: Contents: write 토큰은 main 외 feature 브랜치를 force-push·삭제할 수 있어 다른 워커의 in-flight worktree를 손상시킬 수 있다. → 브랜치 네이밍 규칙(`wt-issue-*`)에 보호 규칙을 적용하거나, 이 위험을 알려진 잔여 위험으로 문서화한다.
- **에이전트의 자기 PR 승인**: Reviewer 에이전트가 Issues/PR write를 가지면 사람 게이트를 우회해 PR을 승인·전이할 수 있다. → Reviewer 토큰 식별자를 PR 작성 토큰과 분리하고, branch protection에 **사람 리뷰 1인 필수**(또는 CODEOWNERS)를 걸어 에이전트 승인만으로는 머지되지 않게 한다. 추가로 branch protection의 **"include administrators"**(관리자도 규칙 적용)를 켜 admin override 머지를 막고, 에이전트 토큰에는 **admin 권한을 부여하지 않으며**, 리뷰 dismiss 권한도 제한한다.
- **Projects write 범위**: Projects: write는 프로젝트 전체 아이템·필드를 수정할 수 있어 단일 이슈를 넘는 blast radius를 갖는다. → 가능하면 에이전트 토큰에서 Projects write를 빼고, label↔status 동기화는 8장의 Project 자동화에 맡긴다.

**요약**: 토큰 격리(blast radius 축소) + branch protection(행동 제약)은 서로를 대체하지 않는 두 개의 층이며, 반드시 함께 적용한다. 그 위에 (3)의 잔여 위험을 개별 대응한다.
...
단, 워커가 작업 도중 죽으면 이슈가 `status:in-progress`에 **고아 락(orphan lock)** 으로 남아 다른 워커가 집지 못한다(상태는 보존되나 진행은 멈춘다). → reclaim 절차를 둔다: claim 시 기록한 타임스탬프(3.3의 claim 토큰)를 기준으로, `in-progress`가 일정 시간(예: 2시간) 이상 갱신 없이 머물면 Triager 또는 별도 sweeper lane이 PR 진척이 없음을 확인한 뒤 `status:agent-ready`로 되돌린다.

**주의(잔여 위험):** 느리지만 살아있는 워커(대형 빌드·느린 CI 대기 등)의 락을 sweeper가 조기 회수하면 두 워커가 같은 이슈를 동시에 진행하는 중복 작업이 생긴다. → 워커가 작업 중 주기적으로 claim 토큰 타임스탬프를 갱신(heartbeat)하게 하고, sweeper는 heartbeat가 timeout을 넘긴 경우에만 회수한다. timeout 값·heartbeat 주기는 9장 open question.
...
> **측정 방법:** 위 지표는 모두 GitHub 데이터에서 추출 가능하다 — 라벨별 이슈/PR 수와 라벨 전이 타임스탬프를 `gh` 쿼리 또는 GitHub Projects insight로 수집한다. **baseline은 Spec/PoC 단계에서 1~2주 측정 후 목표치를 설정**한다(현 PRD 단계에서는 목표 수치를 고정하지 않는다).
...
9. **이슈 유입 경로**: `status:triage` 이슈를 누가/어떻게 생성하는가 (사람 직접 입력 / issue template / 외부 자동화) 및 최초 label 부착 주체 (3.1)
10. **label·Project 부트스트랩**: `status:*` 라벨 7종의 최초 생성(`gh label create` 또는 `.github/labels.yml`)과 GitHub Project↔label v1 동기화 방식(수동 vs `/work-issue` 내 `gh project item-edit` 호출) (3.1)
11. **고아 락 reclaim**: 죽은 워커가 남긴 `status:in-progress` 이슈를 되돌리는 timeout·주체, 및 살아있는 워커 오회수를 막는 heartbeat 주기 (6.6)
